...
SceneManager::~SceneManager()
{
	...
	if (OverlordGame::GetGameSettings().DirectX.EnableDeferredRendering)
		DeferredRenderer::DestroyInstance();
}
...
void SceneManager::Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, OverlordGame* pGame)
{
	...
	if (OverlordGame::GetGameSettings().DirectX.EnableDeferredRendering)
		DeferredRenderer::GetInstance()->InitRenderer(pDevice, pDeviceContext, pGame);
...
void SceneManager::Draw() const
{
	...
	if (OverlordGame::GetGameSettings().DirectX.EnableDeferredRendering)
	{
		...
		DeferredRenderer::GetInstance()->Begin(); //Configures pipeline for Deferred Rendering
		m_ActiveScene->RootDraw(); //Draw Deferred (Opaque Objects only)
		DeferredRenderer::GetInstance()->End(); //Resets pipeline to Forward Rendering
