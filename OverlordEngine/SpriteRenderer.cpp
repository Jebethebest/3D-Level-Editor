...
void SpriteRenderer::InitRenderer(ID3D11Device* pDevice)
{
	...
	m_pEffect = ContentManager::Load<ID3DX11Effect>(L"./Resources/Effects/SpriteExRenderer.fx");
#else
...
{
#ifdef SPRITE_EFFECT_SUPPORT
	Logger::LogWarning(L"SPRITE_EFFECT_SUPPORT is active, use SpriteExComponent instead of SpriteComponent! (Or disable the define [SpriteRenderer])");
	return;
#endif
...
{
#ifdef SPRITE_EFFECT_SUPPORT
	Logger::LogWarning(L"SPRITE_EFFECT_SUPPORT is active, SpriteRenderer::DrawImmediat not supported. (disable the define if you want to use it)");
	return;
#endif
